...
uint32_t vulkanMemoryType(VkMemoryPropertyFlags iProperties, uint32_t iTypeBits) {
    VkPhysicalDeviceMemoryProperties prop;
    vkGetPhysicalDeviceMemoryProperties(Application::GetPhysicalDevice(), &prop);
    for (uint32_t i = 0; i < prop.memoryTypeCount; i++) {
        if ((prop.memoryTypes[i].propertyFlags & iProperties) == iProperties && iTypeBits & (1 << i))
            return i;
    }
    return 0xffffffff;
}

uint32_t bytesPerPixel(ImageFormat iFormat) {
    switch (iFormat) {
...
VkFormat leafFormatToVulkanFormat(ImageFormat iFormat) {
    switch (iFormat) {
        case ImageFormat::RGBA:
...
Image::Image(std::string_view iPath) : _filepath(iPath) {
...
Image::Image(uint32_t iWidth, uint32_t iHeight, ImageFormat iFormat, const void* iData) : _width(iWidth), _height(iHeight), _format(iFormat) {
    allocateMemory(_width * _height * bytesPerPixel(_format));
    if (iData) {
        setData(iData);
    }
...
void Image::setData(const void* iData) {
    ...
    // Copy to Image
    {
        ...
        vkCmdPipelineBarrier(
            command_buffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, nullptr, 0, nullptr, 1, &use_barrier);

        Application::FlushCommandBuffer(command_buffer);
    }
}

void Image::resize(uint32_t iWidth, uint32_t iHeight) {}

void Image::allocateMemory(uint64_t iSize) {
    VkDevice device = Application::GetDevice();

    VkResult err;

    VkFormat vulkanFormat = leafFormatToVulkanFormat(_format);
...
void Image::release() {
    Application::SubmitResourceFree([sampler             = _sampler,
